...
class JobSerializer(serializers.ModelSerializer):
    input_data = serializers.PrimaryKeyRelatedField(
        many=True, required=False, queryset=models.File.objects.all()
    )
    output_data = serializers.PrimaryKeyRelatedField(
        many=True, required=False, queryset=models.File.objects.all()
    )
    ...
    out_file = file.FileSerializer(read_only=True)
    err_file = file.FileSerializer(read_only=True)
...
class JobLogSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = models.Job
        fields = (
            'id', 'saved_id', 'job_status', 'exit_code', 'out_file', 'err_file'
        )
